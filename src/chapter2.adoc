...
one or more Resource Structures.

Each Resource Structure defines the properties of the resource with which the QoS
Controller is associated. A QoS Controller may be associated with multiple resources.
The specific resource associations are implementation-defined. The RQSC table exposes
this information to enable software to determine the appropriate QoS configuration.
